...
{
  ContactFaceFaceInteraction* cffi = NULL;
  ...
  if (all_planar==element->Faces_Per_Element()) {
    ...
    if (in_cnt!=element->Faces_Per_Element()*face->Nodes_Per_Face()) {
      Real tol=std::numeric_limits<double>::epsilon(); // PJSA was 1e-6 
      p = &poly0;
      q = &poly1;
      Poly*  r;
      Point* u;
    ...
    if (p->np>0) {
      ...
      if(tarea > 10*std::numeric_limits<double>::epsilon()) {//HB: avoid ctc polygon of null (small) area
        ...
        cffi = ContactFaceFaceInteraction::new_ContactFaceFaceInteraction(
...
// This variant doesn't actually compute the intersecting polygon
// it justs checks to see if one exists
// also in_cnt and all_planar are passed by reference
template<typename DataType>
bool
ContactSearch::Face_Face_Search_Step1(ContactFace<DataType>* slave_face, 
                                      ContactFace<DataType>* master_face, 
                                      ContactElem<DataType>* element,
                                      VariableHandle POSITION,
                                      int& in_cnt, int& all_planar)
{
  ContactFaceFaceInteraction* cffi = NULL;
  typedef PointStruct<DataType> Point;
  typedef PolyStruct<DataType> Poly;
  ...
  for (i=0; i<element->Faces_Per_Element(); ++i) {
    if (out[i]==face->Nodes_Per_Face()) return false;
  }
...
// this variant assumes that an intersecting polygon exists
// also in_cnt and allplanar are passed in by value as arguments
template<typename DataType>
ContactFaceFaceInteraction*
ContactSearch::Face_Face_Search_Step2(ContactFace<DataType>* slave_face, 
                                      ContactFace<DataType>* master_face, 
                                      ContactElem<DataType>* element,
                                      VariableHandle POSITION, 
                                      int in_cnt, int all_planar)
