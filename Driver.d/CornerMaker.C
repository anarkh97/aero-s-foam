...
{
  if(deg) delete [] deg;
  if(isCorner) delete [] isCorner;
  if(isSafe) delete [] isSafe;
  if(glSafe) delete [] glSafe;
...
SubCornerHandler::markSafeNodes(FSCommPattern<int> *cpat)
{
  ...
  isSafe = new bool[sharedNodes.numConnect()];
  for(int iNeighb = 0; iNeighb < nNeighb; ++iNeighb) {
...
bool
SubCornerHandler::addPotCornerPoints(int numShared, int *allNodes, bool *safe)
{
  if(numShared < dim) {
