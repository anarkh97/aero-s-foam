                                     ...
                                     GeomState *refState, FullSquareMatrix *mel)
{
  ...
  if(sinfo.isDynam() && mel) getWeightedFictitiousForceOnly(weights, geomState, kel, residual, time, refState, NULL, mel, false);
}
      ...
      for(int i=0; i<numNodes; ++i) {
        ...
        if(beta == 0) {
          ...
          if(domain->solInfo().galerkinPodRom) {
            ...
            f = T.transpose()*(M*Tdot*Psidot + (V_n_h).cross(M*V_n_h));
...
void
Domain::getWeightedFictitiousForceOnly(const std::map<int, double> &weights,GeomState &geomState, FullSquareMatrix *kel,
                                       ...
                                       FullSquareMatrix *mel, bool compute_tangents)
{
  ...
  // treatment of discrete inertias
  // XXX currently no lumping applied to thes
  if(firstDiMass != NULL) {
    DMassData *current = firstDiMass;
    while(current != 0) {
      int idof = current->dof;
