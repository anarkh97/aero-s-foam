		...
		GeomType, StateUpdate >::solve() 
{
  ...
  if(domain->solInfo().sensitivity) {
    probDesc->postProcessNLSA(refState, geomState);
    AllSensitivities<double> *allSens = probDesc->getAllSensitivities();
        ...
        case SensitivityInfo::AggregatedStressVMWRTthickness:
          if( numThicknessGroups > 0 ) {
            ...
            if(!allSens->lambdaAggregatedStressVM) {
              ...
              rhsSen = new VecType( probDesc->solVecInfo() );
              for(int i=0; i<domain->numUncon(); ++i) { (*rhsSen)[i] = (*allSens->aggregatedVonMisesWRTdisp)(i); /* std::cerr << (*rhsSen)[i] << "  ";  */}
//              std::cerr << std::endl;
              probDesc->getSolver()->reSolve(*rhsSen);
//              rhsSen->print();
...
//            std::cerr << "print linearstaticWRTthick\n" << allSens->linearstaticWRTthick[0]->col(0) << std::endl;
            for(int iparam=0; iparam< numThicknessGroups; ++iparam) {
...
//              filePrint(stderr, " ... nldynamprobtype (*allSens->aggregatedVonMisesWRTthick)(%d) is %e\n", iparam, (*allSens->aggregatedVonMisesWRTthick)(iparam));
            }
          }
          break;
    ...
    VecType *fullDispBuffer = new VecType(probDesc->solVecInfo());
...
void
NLDynamSolver < OpSolver, VecType, PostProcessor, ProblemDescriptor,
		GeomType, StateUpdate >::computeLambdaNLStressVM(int numStressNodes) 
{
  AllSensitivities<double> *allSens = probDesc->getAllSensitivities();
