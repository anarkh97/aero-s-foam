...
GenSubDomain<double>::makeG()
{
  ...
  for(int i = 0; i < scomm->numT(SComm::mpc); ++i) {  // loop through all potential contact/mpc neighbours
    ...
    //cerr << "sub = " << subNumber << ", i = " << i << ", G[i] = \n"; G[i]->print();
  }
}

template<>
void
GenSubDomain<DComplex>::makeG()
{
  cerr << " *** WARNING: GenSubDomain<DComplex>::makeG() is not implemented \n";
}

template<>
void
GenSubDomain<double>::assembleGtGsolver(GenSparseMatrix<double> *GtGsolver, int flag)
{
  ...
  for(int i = 0; i < scomm->numT(SComm::mpc); ++i) {
    ...
    GenVector<double> d(scomm->lenT(SComm::mpc,i));
    ...
    if((numGroupRBM2 > 0) && (subNumber != scomm->neighbT(SComm::mpc,i))) {
      GenFullM<double> tmp2(numGroupRBM, numGroupRBM2);  // coupling term
    ...
    GenFullM<double> tmp(numGroupRBM, numGroupRBM);
...
template<>
void
GenSubDomain<DComplex>::assembleGtGsolver(GenSparseMatrix<DComplex> *, int flag)
{
  cerr << " *** WARNING: GenSubDomain<DComplex>::assembleGtGsolver() is not implemented \n";
}

template<>
void
GenSubDomain<double>::multG(GenVector<double> &x, double *y, double alpha, int flag)
{
  // flag = 0: y += alpha * G * x
  //        1: y += alpha * Gtilda * x
  double *mpcvec = (double *) alloca(sizeof(double)*numMPC);
...
template<>
void
GenSubDomain<DComplex>::multG(GenVector<DComplex> &, DComplex *, double, int)
{
  cerr << " *** WARNING: GenSubDomain<DComplex>::multG() is not implemented \n";
}

template<>
void 
GenSubDomain<double>::trMultG(double *x, GenVector<double> &y, double alpha, int flag)
{
...
template<>
void
GenSubDomain<DComplex>::trMultG(DComplex *, GenVector<DComplex> &, double, int)
{
  cerr << " *** WARNING: GenSubDomain<DComplex>::trMultG() is not implemented \n";
}
