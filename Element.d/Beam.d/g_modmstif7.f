...
C=====================================================================C
      subroutine gxmodmstif7(g_estif, a, g_a, e, g_e, eframe, 
     *                       g_eframe, ix, g_ix, iy, g_iy, iz, g_iz,
     *                       alphay, g_alphay, alphaz, g_alphaz, 
     *                       c1, g_c1, nu, g_nu, x, g_x, y, g_y, z, 
     *                       g_z, flag)
C=====================================================================C
C                                                                     C
...
C
        integer g_pmax_
        parameter (g_pmax_ = 1)
        integer g_i_, g_p_, ldg_estif, ldg_x, ldg_y, ldg_z, ldg_eframe, 
     *ldg_ix, ldg_e, ldg_nu
        parameter (g_p_=1)
	parameter (ldg_estif=1)
	parameter (ldg_x=1)
	parameter (ldg_y=1)
	parameter (ldg_z=1)
	...
	integer ldg_iy,ldg_iz,ldg_a,ldg_c1,ldg_alphay,ldg_alphaz
        parameter (ldg_iy=1)
	parameter (ldg_iz=1)
	parameter (ldg_a=1)
	parameter (ldg_c1=1)
     ...
     * d5_b, d1_p
        double precision d6_v, d7_v, d8_v, g_estif(ldg_estif, 12, 12), g
     *_t(g_pmax_, 9), g_locke(g_pmax_, 12, 12), g_le(g_pmax_, 12, 12), g
     *_dx(g_pmax_), g_x(ldg_x, 2), g_dy(g_pmax_)
        double precision g_y(ldg_y, 2), g_dz(g_pmax_), g_z(ldg_z, 2), g_
     *d2_w(g_pmax_), g_d1_w(g_pmax_), g_length(g_pmax_), g_eframe(ldg_ef
     *rame, 9), g_jj(g_pmax_), g_ix(ldg_ix), g_g(g_pmax_)
        double precision g_e(ldg_e), g_nu(ldg_nu), g_s11(g_pmax_), g_s22
     *(g_pmax_), g_s33(g_pmax_), g_s26(g_pmax_), g_s35(g_pmax_), g_s44(g
     *_pmax_), g_s55(g_pmax_), g_s66(g_pmax_)
        double precision g_s17(g_pmax_), g_s28(g_pmax_), g_s39(g_pmax_),
     * g_s59(g_pmax_), g_s68(g_pmax_), g_s77(g_pmax_), g_s88(g_pmax_), g
     *_s99(g_pmax_), g_s212(g_pmax_), g_s311(g_pmax_)
        double precision g_s410(g_pmax_), g_s511(g_pmax_), g_s612(g_pmax
     *_), g_s812(g_pmax_), g_s911(g_pmax_), g_s1010(g_pmax_), g_s1111(g_
     *pmax_), g_s1212(g_pmax_), g_iy(ldg_iy), g_iz(ldg_iz)
        double precision g_a(ldg_a), g_c1(ldg_c1), g_b(g_pmax_), g_gamma
     *y(g_pmax_), g_bendy(g_pmax_), g_alphay(ldg_alphay), g_bendcy(g_pma
     *x_), g_gammaz(g_pmax_), g_bendz(g_pmax_), g_alphaz(ldg_alphaz)
        double precision g_bendcz(g_pmax_), g_xke(g_pmax_)
        save g_bendcy, g_gammaz, g_bendz, g_bendcz, g_xke
        save g_s511, g_s612, g_s812, g_s911, g_s1010, g_s1111, g_s1212, 
     *g_b, g_gammay, g_bendy
        save g_s28, g_s39, g_s59, g_s68, g_s77, g_s88, g_s99, g_s212, g_
     *s311, g_s410
        save g_g, g_s11, g_s22, g_s33, g_s26, g_s35, g_s44, g_s55, g_s66
     *, g_s17
        save g_t, g_locke, g_le, g_dx, g_dy, g_dz, g_d2_w, g_d1_w, g_len
     *gth, g_jj
...
C
        ...
        data g_ehfid /0/
C
        call ehsfid(g_ehfid, 'modmstif7','g_modmstif7.f')
C
...
99983     continue
          ...
          do i = 1, 12
            do j = 1, 12
              do g_i_ = 1, g_p_
                g_estif(g_i_, i, j) = g_locke(g_i_, i, j)
              enddo
C--------
            enddo
          enddo
