...
subroutine getlemdmg3d0(ematpro,strntens, damage, csntvoit)
  ...
  !  output:
  !  ------
  !  csntvoit(6,6) : voight form secant modulus
  !
  ! ======================================================================
  ...
  real(8), dimension(6,6), intent(out) :: csntvoit
  ! ====================================
  ! local variable
  ! ==============
  real(8) :: cofa, cofb, epscr
  real(8) :: young, poiss
  real(8), dimension(6,6) :: c0
  real(8) :: damage0

  real(8), dimension(3,3) :: strntemp
  real(8), dimension(3) :: pval
  real(8), dimension(5*3) :: w

  real(8) :: epsstar, f
  ! ====================================

  ! initialize: do not initialize damage
  csntvoit(1:6,1:6)= 0.0d0
  ...
  ! get vergin elastic moduli
  call getelsvoit3d(young,poiss, c0)
     ! input : young,poiss
     ! output : c0

  ! copy low triangle of strain tensor
  strntemp(1:3,1:3)= strntens(1:3,1:3)
  strntemp(1,2)= 0.0d0
  strntemp(1,3)= 0.0d0
  strntemp(2,3)= 0.0d0

  ! compute eigen values
  call ea07cd(strntemp,pval,3,3,w)
     ! input : strntemp,3(m),3(ia),w
     ! output : pval

  ! calculate the equivalent strain and corresponding increment
  epsstar= max(pval(1),0.0d0)**2 + max(pval(2),0.0d0)**2 + max(pval(3),0.0d0)**2 ! consider tension only
  epsstar= dsqrt(epsstar) ! the current equivalent strain
  ...
  ! -------------------------------------------------------
  ! set result: voight form secant modulus
  csntvoit(1:6,1:6)= (1.0d0 - damage) * c0(1:6,1:6)
  ! -------------------------------------------------------



  return
end subroutine getlemdmg3d0
...
!
  ...
  ! calculate the equivalent strain and corresponding increment
  epsstar= max(pval(1),0.0d0)**2 + max(pval(2),0.0d0)**2 + max(pval(3),0.0d0)**2 ! tension only
  epsstar= dsqrt(epsstar) ! the current equivalent strain
  ...
  return
end subroutine getlineardmg3d0