...
!      type                  name                              arguement
!      ----                  ----                              ---------
! 1.  subroutine        elefinthypobt1         (optdmg,optcri,prmdmg, &
!                                               delt,ematpro,nndof,mgaus3,mgqpt1,gqpoin3,gqweigt3,ecord,edisp,evelo, &
!                                               evar1,evoit2,evoit3, &
!                                               evar2,efint)
! 2.  subroutine        elefinthypobt0         (optdmg,optcri,prmdmg, &
!                                               delt,ematpro,mgaus3,mgqpt1,gqpoin3,gqweigt3,ecord,edisp,evelo, &
!                                               evar1,evoit2,evoit3, &
!                                               evar2,efint)
! 3.  subroutine        updhypostrsbt          (optdmg,prmdmg, &
!                                               delt,ematpro,gqpoin,gqweigt,locbvec,ecurn,evelo, &
!                                               damage,sigvoitloc,strnvoitloc, &
!                                               effstrn,effstrs)
! 4.  subroutine        gqfintbt               (delt,ematpro,gqpoin,gqweigt,locbvec,ecurn,sigvoitloc, gqfint)
! 5.  subroutine        getsighypobt1          (optdmg,prmdmg,delt,ematpro,ehleng,zeta,ecurnloc,eveloloc, damage,ipstrn, ipstrsdot)
! 6.  subroutine        getsighypobt2          (ematpro,ecurnloc,eveloloc, tsstrsdot)
! 7.  subroutine        getstrndotbt1          (thick,zeta,ecurnloc,eveloloc, ltens2d,ipstrndot)
! 8.  subroutine        getstrndotbt2          (ecurnloc,eveloloc, tsstrndot)
!
! =========================================================================================================



subroutine elefinthypobt1(optdmg,optcri,prmdmg, &
                          delt,ematpro,nndof,mgaus3,mgqpt1,gqpoin3,gqweigt3,ecord,edisp,evelo, &
                          evar1,evoit2,evoit3, &
                          evar2,efint)
  !=======================================================================
  ...
  ! 5 dof bt shell
  ! --------------
  call elefinthypobt0(optdmg,optcri,prmdmg, &
                      delt,ematpro,mgaus3,mgqpt1,gqpoin3,gqweigt3,ecord,edisp0,evelo0, &
                      evar1,evoit2,evoit3, &
                      evar2,efint0)
        ! input : optdmg,optcri,prmdmg
        !         delt,ematpro,mgaus3,mgqpt1,gqpoin3,gqweigt3,ecord,edisp0,evelo0
        ! inoutput : evar1(eff. strn, damage),evoit2(cauchy, local),evoit3(strain)
        ! output : evar2,efint0
  ! ----------------------------------------------------------------------------------------------
...
subroutine elefinthypobt0(optdmg,optcri,prmdmg, &
                          delt,ematpro,mgaus3,mgqpt1,gqpoin3,gqweigt3,ecord,edisp,evelo, &
                          evar1,evoit2,evoit3, &
                          evar2,efint)
  !=======================================================================
  !  elefinthypobt0 = compute element internal nodal force
  ...
  do igaus=1, mgaus3 
     ...
     ! --------------------------
     call updhypostrsbt(optdmg,prmdmg, &
  ...
  return
end subroutine elefinthypobt0




subroutine updhypostrsbt(optdmg,prmdmg, &
                         delt,ematpro,gqpoin,gqweigt,locbvec,ecurn,evelo, &
                         damage,sigvoitloc,strnvoitloc, &
                         effstrn,effstrs)
  !=======================================================================
  ...
  call getsighypobt1(optdmg,prmdmg,delt,ematpro,ehleng,zeta,ecurnloc,eveloloc, &
                     damage,ipstrnloc, &
                     ipstrslocdot)
     ! input : optdmg,prmdmg,delt,ematpro,ehleng,zeta,ecurnloc,eveloloc
...
subroutine gqfintbt(delt,ematpro,gqpoin,gqweigt,locbvec,ecurn,sigvoitloc, gqfint)
  ...
  real(8), dimension(20,1), intent(out) :: gqfint
  ! ====================================
  ! local variable
  ! ==============
  real(8) :: rk, thick
  ...
  real(8), dimension(20,1) :: gqfintloc
...
!  mx=  -psibar *ipstrsloc(1,1) *gqweigt *dtdpsi ! m_x
!  my=  -psibar *ipstrsloc(2,1) *gqweigt *dtdpsi ! m_y
!  mxy= -psibar *ipstrsloc(3,1) *gqweigt *dtdpsi ! m_xy
...
!                                + bmat1pt(1,inode)*mx + bmat1pt(2,inode)*mxy)  ! m_yi
  ...
  do inode=1, 4
     ...
     locvec(1:5,1)= gqfintloc(iloc+1:iloc+5,1)
     ...
     gqfint(iloc+1:iloc+5,1)= glbvec(1:5,1)
...
subroutine getsighypobt1(optdmg,prmdmg,delt,ematpro,ehleng,zeta,ecurnloc,eveloloc, &
                         damage,ipstrn, &
                         ipstrsdot)
  !=======================================================================
  ...
  case(1) ! lematire damage model
     ...
     call getlemdmg2d0(1,prmdmg,ematpro,etens2d, damage, csntvoit2d,ctanvoit2d,ctantens2d)
        ! input : 1(optpty:p-strs),prmdmg,ematpro,etens2d
        ! inoutput : damage
  ...
  case(2) ! linear softening with scaling
     ...
     ! compute tangent elastic moduli based on damage
     call getlineardmg2d0(1,prmdmg,ematpro,ehleng,etens2d, damage, csntvoit2d,ctanvoit2d,ctantens2d)
        ! input : 1(optpty:p-strs),prmdmg,ematpro,ehleng,etens2d
        ! inoutput : damage
...
!  tsstrsdot(1,1)= 2.0d0 * mu * tsstrndot(1,1)
!  tsstrsdot(2,1)= 2.0d0 * mu * tsstrndot(2,1)
...
subroutine getstrndotbt1(thick,zeta,ecurnloc,eveloloc, ltens2d,ipstrndot)
  ...
  do inode=1, 4
     ...
     ! dx= vx,x= b_xi v_xi + psibar*( b^c _xi v_xi + b_xi theta_yi )
!     dx= dx + bmat1pt(1,inode)*eveloloc(1,inode) &
!              + psibar*( bcmat1pt(1,inode)*eveloloc(1,inode) + bmat1pt(1,inode)*eveloloc(5,inode) )
     ! dy= vy,y= b_yi v_yi + psibar*( b^c _yi v_yi - b_yi theta_xi )
!     dy= dy + bmat1pt(2,inode)*eveloloc(2,inode) &
!              + psibar*( bcmat1pt(2,inode)*eveloloc(2,inode) - bmat1pt(2,inode)*eveloloc(4,inode) )
     ! dxy= 0.50d0*[ b_yi v_xi + b_xi v_yi + psibar*( b^c _xi v_yi + b^c _yi v_xi - b_xi theta_xi + b_yi theta_yi) ]
!     dxy= dxy + 0.50d0*( bmat1pt(2,inode)*eveloloc(1,inode) + bmat1pt(1,inode)*eveloloc(2,inode) & 
!                    + psibar*( bcmat1pt(2,inode)*eveloloc(1,inode) + bcmat1pt(1,inode)*eveloloc(2,inode) &
!                              + bmat1pt(2,inode)*eveloloc(5,inode) - bmat1pt(1,inode)*eveloloc(4,inode) ) )
  ...
  ! ----------------------------------------------------------------
  ! set corotational rate of deformation tensor: dtens= (ltens + ltens^t)
  ! ----------------------------------------
  call symtens(2,ltens2d, dtens)
     ! input : 2(nndex),ltens2d
     ! output : dtens

  ! ----------------------------------------------------------------
  ! set in plane velocity strain rate
  ! ---------------------------------
  ipstrndot(1,1)= dtens(1,1)
  ipstrndot(2,1)= dtens(2,2)
  ipstrndot(3,1)= 2.0d0 * dtens(1,2)
