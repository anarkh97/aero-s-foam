...
C=====================================================================C
      subroutine g_compbbm(g_p_, elm, type, x, g_x, ldg_x, y, g_y, ldg_y
     *, dbm, g_dbm, ldg_dbm, fb, clr, cqr, alpha, fm, rowb, colm, rot, g
     *_rot, ldg_rot, l, g_l, ldg_l, p, g_p, ldg_p, fastcal, kbbm, g_kbbm
     *, ldg_kbbm)
C=====================================================================C
C
C     -----------
...
C
        integer g_pmax_
        parameter (g_pmax_ = 1)
        integer g_i_, g_p_, ldg_kbbm, ldg_l, ldg_p, ldg_x, ldg_y, ldg_db
     *m, ldg_rot
        double precision d9_b, d4_v, d8_b, d4_b, d1_p, d1_w, d7_b, d6_b,
     * d2_v, d3_v
        double precision d5_b, d2_b, d3_b, g_kbbm(ldg_kbbm, 18, 18), g_l
     ...
     *_), g_factorb(g_pmax_), g_factorm(g_pmax_), g_dbm(ldg_dbm, 3, 3), 
     *g_dbmpt1(g_pmax_), g_dbmpt2(g_pmax_), g_dbmpt3(g_pmax_), g_rot(ldg
     *_rot, 6, 6)
        save g_ss23, g_ss31, g_cs12, g_cs23, g_cs31, g_factorb, g_factor
     *m, g_dbmpt1, g_dbmpt2, g_dbmpt3
...
C
          col = colm(i)
          do g_i_ = 1, g_p_
            g_d1_w(g_i_) = dbm(1, 2) * g_p(g_i_, i, 2) + p(i, 2) * g_dbm
     *(g_i_, 1, 2) + dbm(1, 1) * g_p(g_i_, i, 1) + p(i, 1) * g_dbm(g_i_,
     * 1, 1)
          enddo
          d1_w = dbm(1, 1) * p(i, 1) + dbm(1, 2) * p(i, 2)
          do g_i_ = 1, g_p_
            g_dbmpt1(g_i_) = dbm(1, 3) * g_p(g_i_, i, 3) + p(i, 3) * g_d
     *bm(g_i_, 1, 3) + g_d1_w(g_i_)
          enddo
          dbmpt1 = d1_w + dbm(1, 3) * p(i, 3)
C--------
          do g_i_ = 1, g_p_
            g_d1_w(g_i_) = dbm(2, 2) * g_p(g_i_, i, 2) + p(i, 2) * g_dbm
     *(g_i_, 2, 2) + dbm(2, 1) * g_p(g_i_, i, 1) + p(i, 1) * g_dbm(g_i_,
     * 2, 1)
          enddo
          d1_w = dbm(2, 1) * p(i, 1) + dbm(2, 2) * p(i, 2)
          do g_i_ = 1, g_p_
            g_dbmpt2(g_i_) = dbm(2, 3) * g_p(g_i_, i, 3) + p(i, 3) * g_d
     *bm(g_i_, 2, 3) + g_d1_w(g_i_)
          enddo
          dbmpt2 = d1_w + dbm(2, 3) * p(i, 3)
C--------
          do g_i_ = 1, g_p_
            g_d1_w(g_i_) = dbm(3, 2) * g_p(g_i_, i, 2) + p(i, 2) * g_dbm
     *(g_i_, 3, 2) + dbm(3, 1) * g_p(g_i_, i, 1) + p(i, 1) * g_dbm(g_i_,
     * 3, 1)
          enddo
          d1_w = dbm(3, 1) * p(i, 1) + dbm(3, 2) * p(i, 2)
          do g_i_ = 1, g_p_
            g_dbmpt3(g_i_) = dbm(3, 3) * g_p(g_i_, i, 3) + p(i, 3) * g_d
     *bm(g_i_, 3, 3) + g_d1_w(g_i_)
          enddo
          dbmpt3 = d1_w + dbm(3, 3) * p(i, 3)
C--------
C
          do 99981 j = 1, 9
            row = rowb(j)
            do g_i_ = 1, g_p_
              g_d1_w(g_i_) = dbmpt2 * g_l(g_i_, j, 2) + l(j, 2) * g_dbmp
     *t2(g_i_) + dbmpt1 * g_l(g_i_, j, 1) + l(j, 1) * g_dbmpt1(g_i_) + g
     *_kbbm(g_i_, row, col)
            enddo
            d1_w = kbbm(row, col) + dbmpt1 * l(j, 1) + dbmpt2 * l(j, 2)
            do g_i_ = 1, g_p_
              g_kbbm(g_i_, row, col) = dbmpt3 * g_l(g_i_, j, 3) + l(j, 3
     *) * g_dbmpt3(g_i_) + g_d1_w(g_i_)
            enddo
            kbbm(row, col) = d1_w + dbmpt3 * l(j, 3)
C--------
5002        continue
99981     continue
C
...
C
C     open(unit=90,file="kbBM.m")
C     write(90,*) "kbBM=["
C     do 991 i=1,18
C 991 write(90,9) (kbBM(i,j),j=1,18)
C     write(90,*) "     ];"
C     close(90)
