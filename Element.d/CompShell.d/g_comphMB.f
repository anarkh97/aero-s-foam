...
C=====================================================================C
      subroutine g_comphmb(g_p_, elm, type, x, g_x, ldg_x, y, g_y, ldg_y
     *, dmb, g_dmb, ldg_dmb, fb, fm, rowm, colb, rot, g_rot, ldg_rot, lh
     *1, g_lh1, ldg_lh1, lh2, g_lh2, ldg_lh2, lh3, g_lh3, ldg_lh3, ph1, 
     *g_ph1, ldg_ph1, ph2, g_ph2, ldg_ph2, ph3, g_ph3, ldg_ph3, fastcal,
     * khmb, g_khmb, ldg_khmb)
C=====================================================================C
C
C     -----------
C     DECLARATION
...
C
     ...
     *, g_h6(g_pmax_), g_dmb(ldg_dmb, 3, 3), g_mult11(g_pmax_)
...
C
          do g_i_ = 1, g_p_
            g_d1_w(g_i_) = dmb(1, 2) * g_lh1(g_i_, i, 2) + lh1(i, 2) * g
     *_dmb(g_i_, 1, 2) + dmb(1, 1) * g_lh1(g_i_, i, 1) + lh1(i, 1) * g_d
     *mb(g_i_, 1, 1)
          enddo
          d1_w = dmb(1, 1) * lh1(i, 1) + dmb(1, 2) * lh1(i, 2)
          do g_i_ = 1, g_p_
            g_mult11(g_i_) = dmb(1, 3) * g_lh1(g_i_, i, 3) + lh1(i, 3) *
     * g_dmb(g_i_, 1, 3) + g_d1_w(g_i_)
          enddo
          mult11 = d1_w + dmb(1, 3) * lh1(i, 3)
C--------
          do g_i_ = 1, g_p_
            g_d1_w(g_i_) = dmb(2, 2) * g_lh1(g_i_, i, 2) + lh1(i, 2) * g
     *_dmb(g_i_, 2, 2) + dmb(2, 1) * g_lh1(g_i_, i, 1) + lh1(i, 1) * g_d
     *mb(g_i_, 2, 1)
          enddo
          d1_w = dmb(2, 1) * lh1(i, 1) + dmb(2, 2) * lh1(i, 2)
          do g_i_ = 1, g_p_
            g_mult12(g_i_) = dmb(2, 3) * g_lh1(g_i_, i, 3) + lh1(i, 3) *
     * g_dmb(g_i_, 2, 3) + g_d1_w(g_i_)
          enddo
          mult12 = d1_w + dmb(2, 3) * lh1(i, 3)
C--------
          do g_i_ = 1, g_p_
            g_d1_w(g_i_) = dmb(3, 2) * g_lh1(g_i_, i, 2) + lh1(i, 2) * g
     *_dmb(g_i_, 3, 2) + dmb(3, 1) * g_lh1(g_i_, i, 1) + lh1(i, 1) * g_d
     *mb(g_i_, 3, 1)
          enddo
          d1_w = dmb(3, 1) * lh1(i, 1) + dmb(3, 2) * lh1(i, 2)
          do g_i_ = 1, g_p_
            g_mult13(g_i_) = dmb(3, 3) * g_lh1(g_i_, i, 3) + lh1(i, 3) *
     * g_dmb(g_i_, 3, 3) + g_d1_w(g_i_)
          enddo
          mult13 = d1_w + dmb(3, 3) * lh1(i, 3)
C--------
C
          do g_i_ = 1, g_p_
            g_d1_w(g_i_) = dmb(1, 2) * g_lh2(g_i_, i, 2) + lh2(i, 2) * g
     *_dmb(g_i_, 1, 2) + dmb(1, 1) * g_lh2(g_i_, i, 1) + lh2(i, 1) * g_d
     *mb(g_i_, 1, 1)
          enddo
          d1_w = dmb(1, 1) * lh2(i, 1) + dmb(1, 2) * lh2(i, 2)
          do g_i_ = 1, g_p_
            g_mult21(g_i_) = dmb(1, 3) * g_lh2(g_i_, i, 3) + lh2(i, 3) *
     * g_dmb(g_i_, 1, 3) + g_d1_w(g_i_)
          enddo
          mult21 = d1_w + dmb(1, 3) * lh2(i, 3)
C--------
          do g_i_ = 1, g_p_
            g_d1_w(g_i_) = dmb(2, 2) * g_lh2(g_i_, i, 2) + lh2(i, 2) * g
     *_dmb(g_i_, 2, 2) + dmb(2, 1) * g_lh2(g_i_, i, 1) + lh2(i, 1) * g_d
     *mb(g_i_, 2, 1)
          enddo
          d1_w = dmb(2, 1) * lh2(i, 1) + dmb(2, 2) * lh2(i, 2)
          do g_i_ = 1, g_p_
            g_mult22(g_i_) = dmb(2, 3) * g_lh2(g_i_, i, 3) + lh2(i, 3) *
     * g_dmb(g_i_, 2, 3) + g_d1_w(g_i_)
          enddo
          mult22 = d1_w + dmb(2, 3) * lh2(i, 3)
C--------
          do g_i_ = 1, g_p_
            g_d1_w(g_i_) = dmb(3, 2) * g_lh2(g_i_, i, 2) + lh2(i, 2) * g
     *_dmb(g_i_, 3, 2) + dmb(3, 1) * g_lh2(g_i_, i, 1) + lh2(i, 1) * g_d
     *mb(g_i_, 3, 1)
          enddo
          d1_w = dmb(3, 1) * lh2(i, 1) + dmb(3, 2) * lh2(i, 2)
          do g_i_ = 1, g_p_
            g_mult23(g_i_) = dmb(3, 3) * g_lh2(g_i_, i, 3) + lh2(i, 3) *
     * g_dmb(g_i_, 3, 3) + g_d1_w(g_i_)
          enddo
          mult23 = d1_w + dmb(3, 3) * lh2(i, 3)
C--------
C
          do g_i_ = 1, g_p_
            g_d1_w(g_i_) = dmb(1, 2) * g_lh3(g_i_, i, 2) + lh3(i, 2) * g
     *_dmb(g_i_, 1, 2) + dmb(1, 1) * g_lh3(g_i_, i, 1) + lh3(i, 1) * g_d
     *mb(g_i_, 1, 1)
          enddo
          d1_w = dmb(1, 1) * lh3(i, 1) + dmb(1, 2) * lh3(i, 2)
          do g_i_ = 1, g_p_
            g_mult31(g_i_) = dmb(1, 3) * g_lh3(g_i_, i, 3) + lh3(i, 3) *
     * g_dmb(g_i_, 1, 3) + g_d1_w(g_i_)
          enddo
          mult31 = d1_w + dmb(1, 3) * lh3(i, 3)
C--------
          do g_i_ = 1, g_p_
            g_d1_w(g_i_) = dmb(2, 2) * g_lh3(g_i_, i, 2) + lh3(i, 2) * g
     *_dmb(g_i_, 2, 2) + dmb(2, 1) * g_lh3(g_i_, i, 1) + lh3(i, 1) * g_d
     *mb(g_i_, 2, 1)
          enddo
          d1_w = dmb(2, 1) * lh3(i, 1) + dmb(2, 2) * lh3(i, 2)
          do g_i_ = 1, g_p_
            g_mult32(g_i_) = dmb(2, 3) * g_lh3(g_i_, i, 3) + lh3(i, 3) *
     * g_dmb(g_i_, 2, 3) + g_d1_w(g_i_)
          enddo
          mult32 = d1_w + dmb(2, 3) * lh3(i, 3)
C--------
          do g_i_ = 1, g_p_
            g_d1_w(g_i_) = dmb(3, 2) * g_lh3(g_i_, i, 2) + lh3(i, 2) * g
     *_dmb(g_i_, 3, 2) + dmb(3, 1) * g_lh3(g_i_, i, 1) + lh3(i, 1) * g_d
     *mb(g_i_, 3, 1)
          enddo
          d1_w = dmb(3, 1) * lh3(i, 1) + dmb(3, 2) * lh3(i, 2)
          do g_i_ = 1, g_p_
            g_mult33(g_i_) = dmb(3, 3) * g_lh3(g_i_, i, 3) + lh3(i, 3) *
     * g_dmb(g_i_, 3, 3) + g_d1_w(g_i_)
          enddo
          mult33 = d1_w + dmb(3, 3) * lh3(i, 3)
C--------
C
          do 99942 j = 1, 9
            row = rowm(j)
            do g_i_ = 1, g_p_
              g_d1_w(g_i_) = mult12 * g_ph1(g_i_, j, 2) + ph1(j, 2) * g_
     *mult12(g_i_) + mult11 * g_ph1(g_i_, j, 1) + ph1(j, 1) * g_mult11(g
     *_i_) + g_khmb(g_i_, row, col)
            enddo
            d1_w = khmb(row, col) + mult11 * ph1(j, 1) + mult12 * ph1(j,
     * 2)
            do g_i_ = 1, g_p_
              g_d2_w(g_i_) = mult21 * g_ph2(g_i_, j, 1) + ph2(j, 1) * g_
     *mult21(g_i_) + mult13 * g_ph1(g_i_, j, 3) + ph1(j, 3) * g_mult13(g
     *_i_) + g_d1_w(g_i_)
            enddo
            d2_w = d1_w + mult13 * ph1(j, 3) + mult21 * ph2(j, 1)
            do g_i_ = 1, g_p_
              g_d3_w(g_i_) = mult23 * g_ph2(g_i_, j, 3) + ph2(j, 3) * g_
     *mult23(g_i_) + mult22 * g_ph2(g_i_, j, 2) + ph2(j, 2) * g_mult22(g
     *_i_) + g_d2_w(g_i_)
            enddo
            d3_w = d2_w + mult22 * ph2(j, 2) + mult23 * ph2(j, 3)
            do g_i_ = 1, g_p_
              g_d4_w(g_i_) = mult32 * g_ph3(g_i_, j, 2) + ph3(j, 2) * g_
     *mult32(g_i_) + mult31 * g_ph3(g_i_, j, 1) + ph3(j, 1) * g_mult31(g
     *_i_) + g_d3_w(g_i_)
            enddo
            d4_w = d3_w + mult31 * ph3(j, 1) + mult32 * ph3(j, 2)
            do g_i_ = 1, g_p_
              g_khmb(g_i_, row, col) = mult33 * g_ph3(g_i_, j, 3) + ph3(
     *j, 3) * g_mult33(g_i_) + g_d4_w(g_i_)
            enddo
            khmb(row, col) = d4_w + mult33 * ph3(j, 3)
C--------
6002        continue
99942     continue
C
...
C
C     open(unit=90,file="khMB.m")
C     write(90,*) "khMB=["
C     do 991 i=1,18
C 991 write(90,9) (khMB(i,j),j=1,18)
C     write(90,*) "     ];"
C     close(90)
