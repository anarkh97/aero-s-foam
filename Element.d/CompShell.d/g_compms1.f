...
C=====================================================================C
      subroutine gxcompms1(x,g_x,y,g_y,z,g_z,h,g_h,rho,g_rho,emass,
     *                     medof,nttly,ncmpfr,elm,idlay,mtlay,g_mtlay,
     *                     cmpfr,iatt,ctyp,catt,cfrm,gamma,grvfor,
     *                     grvflg,totmas,g_totmas,masflg)
C=====================================================================C
C                                                                     C
...
C
        integer g_pmax_
        parameter (g_pmax_ = 1)
        integer g_i_, g_p_, ldg_mtlay, ldg_x, ldg_y, ldg_z, ldg_h, ldg_r
     *ho, ldg_totmas
c
c manually inserted - begin	     
c
        parameter (g_p_=1,ldg_x=1,ldg_y=1,ldg_z=1,ldg_h=1,ldg_rho=1,
     *             ldg_mtlay=1,ldg_totmas=1)
c
c manually inserted - end      
c
        double precision d2_b, d6_b, d3_b, d1_w, d1_p, d5_b, d4_b, d2_w,
     * d2_v, d3_v
        double precision d4_v, g_mass0(g_pmax_), g_rholayer(g_pmax_, max
     *layer), g_hlayer(g_pmax_, maxlayer), g_mtlay(ldg_mtlay, 9, nttly),
     * g_x21(g_pmax_), g_x(ldg_x, 3), g_y21(g_pmax_), g_y(ldg_y, 3), g_z
     *21(g_pmax_)
        double precision g_z(ldg_z, 3), g_x32(g_pmax_), g_y32(g_pmax_), 
     *g_z32(g_pmax_), g_d2_w(g_pmax_), g_d1_w(g_pmax_), g_rlr(g_pmax_), 
     *g_rlb(g_pmax_), g_bpr(g_pmax_), g_twicearea2(g_pmax_)
        double precision g_area(g_pmax_), g_thick(g_pmax_), g_h(ldg_h, 3
     *), g_rho(ldg_rho), g_rhoh(g_pmax_), g_totmas(ldg_totmas)
        save g_d1_w, g_rlr, g_rlb, g_bpr, g_twicearea2, g_area, g_thick,
     * g_rhoh
        save g_mass0, g_rholayer, g_hlayer, g_x21, g_y21, g_z21, g_x32, 
     *g_y32, g_z32, g_d2_w
...
C
        if (masflg) then
          do g_i_ = 1, g_p_
            g_totmas(g_i_) = 3.0d0 * g_mass0(g_i_) + g_totmas(g_i_)
          enddo
