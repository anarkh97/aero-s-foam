#include <Element.d/Joint.d/DistanceConstraint.h>
#include <Driver.d/Mpc.h>

DistanceConstraint::DistanceConstraint(int* _nn)
 : ConstraintElement(2, DofSet::XYZdisp, _nn)
{
}

void 
DistanceConstraint::computeMPCs(CoordSet& cs)
{
  ...
  if(l0 == 0.0) {
    cerr << " *** ERROR: division by zero in DistanceConstraint::computeMPCs between nodes " << nn[0]+1 << " and " << nn[1]+1 << endl;
    exit(-1);
  }
/*
  for(int i = 0; i < 3; ++i)
    addterm(new LMPCTerm(nn[0], i, -d0[i]/l0));
  for(int i = 0; i < 3; ++i)
    addterm(new LMPCTerm(nn[1], i, d0[i]/l0));
*/
  for(int i = 0; i < 3; ++i) {
...
int 
DistanceConstraint::getTopNumber() 
{ 
  return 106; 
}

void 
DistanceConstraint::updateLMPCs(GeomState& gState, CoordSet& cs)
{
...
void
DistanceConstraint::getHessian(GeomState& gState, CoordSet& cs, FullSquareMatrix& H)
{
  // nodes' current coordinates
  NodeState ns1 = gState[nn[0]];
  NodeState ns2 = gState[nn[1]];
