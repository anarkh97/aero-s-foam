...
DotConstraintType1::DotConstraintType1(int* _nn, int _axis1, int _axis2)
 : ConstraintElement(2, DofSet::XYZrot, _nn)
{
  elemframe = 0;
  axis1 = _axis1;
  axis2 = _axis2;
...
void 
DotConstraintType1::computeMPCs(CoordSet& cs)
{
  ...
  if(elemframe) {
    ...
    if(l0 == 0.0) {
      cerr << " *** ERROR: division by zero in DotConstraintType1 between nodes " << nn[0]+1 << " and " << nn[1]+1 << endl;
      exit(-1);
    }
...
/*
  // create mpc with zero coefficients and rhs
  for(int i = 0; i < 3; ++i) 
    addterm(new LMPCTerm(nn[0], 3+i, 0.0));
  for(int i = 0; i < 3; ++i)  
    addterm(new LMPCTerm(nn[1], 3+i, 0.0));
*/
...
void 
DotConstraintType1::updateLMPCs(GeomState& gState, CoordSet& cs)
{
