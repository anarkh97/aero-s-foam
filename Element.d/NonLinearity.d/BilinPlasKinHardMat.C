...
(tm)[1][1]=lambdadivnu*(1-2*nu);
(tm)[2][2]=lambdadivnu*(1-2*nu);
(tm)[3][3]=lambdadivnu*(1-nu);
(tm)[4][4]=lambdadivnu*(1-2*nu);
...
void
BilinPlasKinHardMat::integrate(Tensor *_stress, Tensor *_tm,Tensor &_en,Tensor  &_enp, double *staten, double *statenp, double){
...
Tensor_d0s4_Ss12s34 &tm = static_cast<Tensor_d0s4_Ss12s34 &>(*_tm); 
//Tensor_d0s2_Ss12 & en = static_cast<Tensor_d0s2_Ss12 &>(_en);
Tensor_d0s2_Ss12 & enp = static_cast<Tensor_d0s2_Ss12 &>(_enp);
Tensor_d0s2_Ss12 & stress = static_cast<Tensor_d0s2_Ss12 &>(*_stress); 
...
double Hprime = (E*Ep)/(E-Ep);

Tensor_d0s4_Ss12s34 tmel;
...
if (trialyieldnp <= 0){
  ...
  Tensor_d0s4_Ss12s34 tmtest;
  for (i=0; i<3; ++i)
    for (j=i; j<3; ++j)
      for (k=0; k<3; ++k)
        for (l=k; l<3; ++l)
          (tmtest)[i*(5-i)/2+j][k*(5-k)/2+l] = (bulk*delta(i,j)*delta(k,l))                                       +(2*mu*thetanp*((delta(i,k)*delta(j,l)+delta(i,l)*delta(j,k))/2.-delta(i,j)*delta(k,l)/3.)) - 2*mu*thetaprimenp*normalnp[i*(5-i)/2+j]*normalnp[k*(5-k)/2+l];
  tm.reOrder(tmtest);
