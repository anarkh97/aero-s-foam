...
class GaussIntgElement : public MatNLElement {
   ...
   public:
     ...
     void integrate(Node *nodes, double *dispn,  double *staten,
                         ...
                         double *force, double dt=0.0);
     int numStates() { //return getNumGaussPoints()*getMaterial()->getNumStates(); }
                int nGP = getNumGaussPoints();
		Material *mat = getMaterial();
		int nsGP = mat->getNumStates(); 
                return nGP*nsGP; } // PJSA 10-20-08, this function was previously not returning anything
...
FullSquareMatrix  
GenGaussIntgElement<TensorType>::massMatrix(CoordSet& ,double *k,int)
{
  return FullSquareMatrix(numDofs(), k);
}
