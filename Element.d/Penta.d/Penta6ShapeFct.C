...
// HB (09-23-03): shape fct & derivative for the 6 nodes pentahedral element
void
Penta6ShapeFct(double Shape[6], double DShape[6][3], double* J, double m[3], double X[6], double Y[6], double Z[6])
{
  double r = m[0]; // = x
  double s = m[1]; // = y
  double z = m[2];
  ...
  dShape[0][0] = -zm ; dShape[0][1] = -zm ; dShape[0][2] = -t;
  dShape[1][0] =  zm ; dShape[1][1] =  0.0; dShape[1][2] = -r; 
  dShape[2][0] =  0.0; dShape[2][1] =  zm ; dShape[2][2] = -s;
  dShape[3][0] = -zp ; dShape[3][1] = -zp ; dShape[3][2] =  t;
  dShape[4][0] =  zp ; dShape[4][1] =  0.0; dShape[4][2] =  r;
  dShape[5][0] =  0.0; dShape[5][1] =  zp ; dShape[5][2] =  s;

  // -------> JACOBIAN MATRIX (MAPPING FROM REF TO REAL ELEMENT)
  
  xd1 = 0.0 ; xd2 = 0.0 ; xd3 = 0.0 ;
  yd1 = 0.0 ; yd2 = 0.0 ; yd3 = 0.0 ;
  zd1 = 0.0 ; zd2 = 0.0 ; zd3 = 0.0 ;
 
  xd1 = (X[1]-X[0])*A + (X[4]-X[3])*B ;
  yd1 = (Y[1]-Y[0])*A + (Y[4]-Y[3])*B ;
  zd1 = (Z[1]-Z[0])*A + (Z[4]-Z[3])*B ;

  xd2 = (X[2]-X[0])*A + (X[5]-X[3])*B ;
  yd2 = (Y[2]-Y[0])*A + (Y[5]-Y[3])*B ;
  zd2 = (Z[2]-Z[0])*A + (Z[5]-Z[3])*B ;

  xd3 = (X[3]-X[0])*t + (X[4]-X[1])*r + (X[5]-X[2])*s ;
  yd3 = (Y[3]-Y[0])*t + (Y[4]-Y[1])*r + (Y[5]-Y[2])*s ;
  zd3 = (Z[3]-Z[0])*t + (Z[4]-Z[1])*r + (Z[5]-Z[2])*s ;
