...
FourNodeQuad3D::FourNodeQuad3D(int* nodenums)
{
...
void
FourNodeQuad3D::renum(int *table)
{
  for(int i = 0; i < numNodes(); ++i)
    nn[i] = table[nn[i]];
}
...
int
FourNodeQuad3D::numNodes()
{
  return 4;
}

int*
FourNodeQuad3D::nodes(int *p)
{
...
int
FourNodeQuad3D::numDofs()
{
  return 3*numNodes();
}

int*
FourNodeQuad3D::dofs(DofSetArray &dsa, int *p)
{
...
void
FourNodeQuad3D::markDofs(DofSetArray &dsa)
{
  dsa.mark(nn, numNodes(), DofSet::XYZdisp);
}

FullSquareMatrix
FourNodeQuad3D::stiffness(CoordSet &cs, double *d, int flg)
{
...
FullSquareMatrix
FourNodeQuad3D::massMatrix(CoordSet &cs, double *mel, int cmflg)
{
...
double
FourNodeQuad3D::getMass(CoordSet& cs)
{
  return 0.0;
}

void
FourNodeQuad3D::getGravityForce(CoordSet& cs, double *gravityAcceleration,
...
void
FourNodeQuad3D::getVonMises(Vector& stress, Vector& weight, CoordSet &cs,
...
void
FourNodeQuad3D::getAllStress(FullM& stress, Vector& weight, CoordSet &cs,
                             Vector& elDisp, int strInd, int, double *ndTemps)
{
  stress.zero();
  weight.zero();
...
void
FourNodeQuad3D::computeDisp(CoordSet &cs, State &state, const InterpPoint &ip,
...
void
FourNodeQuad3D::getFlLoad(CoordSet &cs, const InterpPoint &ip, double *flF, 
                          double *resF, GeomState *gs)
{
  const double *gp = ip.xy;
  for(int i = 0; i < 3; ++i) {
    resF[i]   = (1-gp[0])*(1-gp[1])* flF[i];
    resF[3+i] = gp[0]*(1-gp[1])* flF[i];
    resF[6+i] = gp[0]*gp[1]* flF[i];
    resF[9+i] = (1-gp[0])*gp[1]* flF[i];
  }
}

void
FourNodeQuad3D::computePressureForce(CoordSet& cs, Vector& elPressureForce,
                                     GeomState *geomState, int cflg)
{
  ...
  int optele = 3;
  _FORTRAN(elefbc3dbrkshl2)(opttrc, optele, ecord, edisp, trac, elPressureForce.data());
}

int
FourNodeQuad3D::getTopNumber()
{
  return 102;
}
