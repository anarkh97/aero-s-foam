#ifdef USE_EIGEN3
#ifndef _THREENODESHELLSTRESSWRTNODALCOORDINATESENSITIVITY_H_
#define _THREENODESHELLSTRESSWRTNODALCOORDINATESENSITIVITY_H_

#include <Element.d/Function.d/Function.h>
#include <Element.d/Shell.d/ShellElementSemiTemplate.cpp>

// class template to facilitate computation of the sensitivities of the nodal von mises stress w.r.t nodal coordinate

template<typename Scalar>
class ThreeNodeShellStressWRTNodalCoordinateSensitivity : public VectorValuedFunction<9,3,Scalar,21,1,double>
{
  public:
    ShellElementSemiTemplate<Scalar> ele;
    Eigen::Array<Scalar,18,1> globalu;
    Eigen::Array<Scalar,3,1> ndTemp; 
    Scalar E, nu, W, h, Ta; // material properties
    int surface; // thru-thickness location at which stresses are to be evaluated
    int isTemp;

  public:
    ThreeNodeShellStressWRTNodalCoordinateSensitivity(const Eigen::Array<double,21,1>& sconst, const Eigen::Array<int,1,1>& iconst)
    {
    ...
    Eigen::Matrix<Scalar,3,1> operator() (const Eigen::Matrix<Scalar,9,1>& q, Scalar)
    {
      ...
      Eigen::Matrix<Scalar,3,1> globalx;
      Eigen::Matrix<Scalar,3,1> globaly;
      Eigen::Matrix<Scalar,3,1> globalz;
      globalx << q[0], q[3], q[6];
      globaly << q[1], q[4], q[7];
      globalz << q[2], q[5], q[8];
