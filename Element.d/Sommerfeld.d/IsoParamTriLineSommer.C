...
int* IsoParamTriLineSommer::dofs(DofSetArray &dsa, int *p) {
...
int* IsoParamTriLineSommer::wetDofs(DofSetArray &dsa, int *p) {
 ...
 int i;
 for(i=0;i<order;i++) dsa.number(nn[i],
...
FullSquareMatrix IsoParamTriLineSommer::sommerMatrix(CoordSet &cs, double *d) {
...
FullSquareMatrix IsoParamTriLineSommer::turkelMatrix(CoordSet &cs, double *d) {
...
void
IsoParamTriLineSommer::markDofs(DofSetArray &dsa)
{
