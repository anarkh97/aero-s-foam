...
int* IsoParamTriSommer::solidDofs(DofSetArray &dsa, int *p) {
 ...
 int i;
 for(i=0;i<ordersq;i++) dsa.number(nn[i],
...
void IsoParamTriSommer::wetInterfaceVector(CoordSet& cs, ComplexVector& cv,
 ...
 if (coefi[0] == complex<double>(0.0,0.0) &&
     coefi[1] == complex<double>(0.0,0.0)) imat = 2;
...
//#define _USE3D_EL
#ifdef _USE3D_EL 
FullSquareMatrix IsoParamTriSommer::sommerMatrix(CoordSet &cs, double *d) {
...
FullSquareMatrix IsoParamTriSommer::turkelMatrix(CoordSet &cs, double *d) {
