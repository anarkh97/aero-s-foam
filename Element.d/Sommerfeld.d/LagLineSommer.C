...
//     cv[j]+= ComplexD(0.0,si*gw[i]*kappa*(-tau[1]*dx+tau[0]*dy))*
     cv[j]+= ComplexD(0.0,gw[i]*kappa*((-tau[1]*dx+tau[0]*dy)-sqrt(tau[0]*tau[0]+tau[1]*tau[1])))*
...
FullSquareMatrix LagLineSommer::sommerMatrix(CoordSet &cs, double *d) {
 ...
 for(i=0;i<ng;i++) { 
   ...
   int j;
   for(j=0;j<order;j++) for(k=0;k<order;k++) 
...
FullSquareMatrix LagLineSommer::turkelMatrix(CoordSet &cs, double *d) {
