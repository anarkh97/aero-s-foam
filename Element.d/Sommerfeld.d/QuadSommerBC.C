...
FullSquareMatrix
QuadSommerBC::sommerMatrix(CoordSet &cs, double *d)
{
	double x[4], y[4], z[4];
...
FullSquareMatrix
QuadSommerBC::turkelMatrix(CoordSet &cs, double *d)
{
        double x[4], y[4], z[4];
...
void 
QuadSommerBC::getLocalCoordinates (CoordSet &cs, double xx[4], double yy[4], double zz[4])
{
	...
	// Make eta local axis connecting nodes 1 and 4
        ...
        eta[0] = (x[3]-x[0])/l14; eta[1] = (y[3]-y[0])/l14; eta[2] = (z[3]-z[0])/l14;
...
void QuadSommerBC::SurfaceRefinement(int nNo, double* x, double* y, double* z, double* xx, double* yy, double* zz)
{
  ...
  u[0]=-1.0; u[1]=1.0; u[2]=1.0; u[3]=-1.0; u[4]=-1.0+dd; u[5]=1.0-dd; u[6]=1.0-dd; u[7]=-1.0+dd;
  v[0]=-1.0; v[1]=-1.0; v[2]=1.0; v[3]=1.0; v[4]=-1.0+dd; v[5]=-1.0+dd; v[6]=1.0-dd; v[7]=1.0-dd;
  for (i=0;i<nNo;i++) {
    xx[i] = x[i];
    xx[i+nNo] = xart[i];
    yy[i] = y[i];
