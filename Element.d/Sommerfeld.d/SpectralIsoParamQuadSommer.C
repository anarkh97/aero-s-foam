...
int* SpectralIsoParamQuadSommer::wetDofs(DofSetArray &dsa, int *p) {
 ...
 int i;
 for(i=0;i<order*order;i++) dsa.number(nn[i],
...
FullSquareMatrix SpectralIsoParamQuadSommer::sommerMatrix(CoordSet &cs, double *d) {
...
FullSquareMatrix SpectralIsoParamQuadSommer::turkelMatrix(CoordSet &cs, double *d) {
...
void SpectralIsoParamQuadSommer::getNormal(CoordSet &cs, double normal[3]) {
