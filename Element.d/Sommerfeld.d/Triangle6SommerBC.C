...
Triangle6SommerBC::Triangle6SommerBC(int n1, int n2, int n3, int n4, int n5, int n6, Element *_el,int etype):SommerElement(_el)
{
  nn[0] = n1;
  nn[1] = n2; 
  nn[2] = n3; 
 ...
 for(j=0;j<6;j++) for(k=j;k<6;k++) {
   ...
   for(i=0;i<nint;i++) {
     ...
     ComplexD tmp2 = ComplexD(HH,0.0) + ComplexD(0.0,1.0/2.0/kappa)/
                       ComplexD(1.0,2*HH/kappa) * (KK-HH*HH)/*- ComplexD(bl/4.0/kappa/kappa,0.0)*/;
     sm[j][k] += tmp1 * tmp2;
   }
 }
...
Triangle6SommerBC::getLocalCoordinatesNew (CoordSet &cs, double T[3][3], double xi1[3], double xi2[3], double xi3[3] ) 
{
	...
	*/
	...
	T[2][0] =  nu[0]; T[2][1] =  nu[1]; T[2][2] =  nu[2];
        ...
        xi2[0] = eta[0]; xi2[1] = eta[1]; xi2[2] = eta[2];
        xi3[0] = nu[0]; xi3[1] = nu[1]; xi3[2] = nu[2];
}



FullSquareMatrix
Triangle6SommerBC::sommerMatrix(CoordSet &cs, double *d)
{
	double x[6], y[6], z[6];
...
FullSquareMatrix
Triangle6SommerBC::turkelMatrix(CoordSet &cs, double *d)
{
        double x[6], y[6], z[6];
...
void 
Triangle6SommerBC::getLocalCoordinates (CoordSet &cs, double xx[6], double yy[6], double zz[6])
{
...
/*
int* Triangle6SommerBC::nodes(int *p)
{
        if(p == 0) p = new int[6];
        p[0] = nn[0];
        p[1] = nn[1];
