                 ...
                 internalDI(_nsub), interface(_nsub), times(threadManager->numThr(), _nsub)
{
 ...
 for(int iSub=0; iSub<nsub; ++iSub) sd[iSub]->setDofCommSize(vPat);
 vPat->finalize();
 ...
 for(int iSub=0; iSub<nsub; ++iSub) sd[iSub]->setCommSize(sPat, 1);
 sPat->finalize();
 ...
 int halfInterfLen = 0;
 for(iSub = 0; iSub < nsub; ++iSub) {
   interface.domLen[iSub] = sd[iSub]->interfLen();
   internalDI.domLen[iSub]  = sd[iSub]->numUncon();

   sd[iSub]->computeMasterFlag(mpcToSub);
   fetiOps[iSub]->setHalfOffset(halfInterfLen);
   halfInterfLen += sd[iSub]->halfInterfLen();
   tInterfLen    += sd[iSub]->interfLen();
   tLocalLen     += sd[iSub]->localLen();
 }
 ...
 for(iSub = 0; iSub < nsub; ++iSub) {
   bool *subMasterFlag = sd[iSub]->getMasterFlag();
...
{
 sd[iSub]->fSend(force.subData(sd[iSub]->localSubNum()), vPat);
}
...
{
 sd[iSub]->fScale(force.subData(sd[iSub]->localSubNum()), vPat);
}
...
{
 sd[iSub]->fSend(force.subData(sd[iSub]->localSubNum()), vPat,
                 fw.subData(sd[iSub]->localSubNum()));
}
...
{
 sd[iSub]->splitInterf(force.subData(sd[iSub]->localSubNum()));
}
...
template<class Scalar>
void
GenFetiSolver<Scalar>::orthoAdd(GenDistrVector<Scalar> &p, GenDistrVector<Scalar> &Fp, Scalar pFp)
{
...
{
  sd[iSub]->rebuildInterf(v.subData(sd[iSub]->localSubNum()), vPat);
}
...
{
 sd[iSub]->interfaceJump(v.subData(sd[iSub]->localSubNum()), vPat);
}
...
{
  Scalar *v1              =         v.subData(sd[iSub]->localSubNum());
  Scalar *interfaceVector = interfvec.subData(sd[iSub]->localSubNum());
  Scalar *subDeltaU       =    deltaU.subData(sd[iSub]->localSubNum());
  Scalar *subDeltaF       = deltaF.subData(sd[iSub]->localSubNum());
                                           ...
                                           int isNonlinear, int numrbms, int numcrns)
{
  ...
  Fr      = new GenDistrVector<Scalar>(interface);
...
template<class Scalar>
void
GenFetiWorkSpace<Scalar>::save_lambda() 
{
  if(!lambda_copy) lambda_copy = new GenDistrVector<Scalar>(*lambda); else *lambda_copy = *lambda;
}
...
GenFetiSolver<Scalar>::gatherHalfInterface(int iSub, GenDistrVector<Scalar> *v1, GenDistrVector<Scalar> *v2,
                                           Scalar *v3, Scalar *v4)
{
 Scalar *vec1 = v1->subData(sd[iSub]->localSubNum());
 Scalar *vec2 = (v2) ? v2->subData(sd[iSub]->localSubNum()) : 0;

 Scalar *vec3 = v3 + halfOffset(iSub);
 Scalar *vec4 = v4 + halfOffset(iSub);

 if(vec2)
   sd[iSub]->getHalfInterf(vec1, vec3, vec2, vec4);
 else
   sd[iSub]->getHalfInterf(vec1, vec3);
}
...
GenFetiSolver<Scalar>::scatterHalfInterface(int iSub, Scalar *v1, GenDistrVector<Scalar>* v2)
{
  ...
  Scalar *vec2 = v2->subData(sd[iSub]->localSubNum());

  sd[iSub]->scatterHalfInterf(vec1, vec2);
  sd[iSub]->sendInterf(vec2, vPat);
}
...
GenFetiSolver<Scalar>::factorMatrices(int iSub)
{
 ...
 if(fop->rbm) {
   ...
   if(fop->numRBM > 0) {
      fop->locRBMs = new double[fop->numRBM * sd[iSub]->localLen()];
                                  ...
                                  GenDistrVector<Scalar> &v3, GenDistrVector<Scalar> &v4)
{
 ...
 int i;
 for(i = 0; i < sd[iSub]->localLen(); ++i)
    localvec[i] = localsrc[i];

 for(i = 0; i < sd[iSub]->interfLen(); ++i)
...
{
 Scalar *interfvec = dv1.subData(sd[iSub]->localSubNum());
 sd[iSub]->sendInterf(interfvec, vPat);
}
...
{
 // Difference
 Scalar *interfvec = dv2.subData(sd[iSub]->localSubNum());
 sd[iSub]->interfaceJump(interfvec, vPat);

 // Dot
 fetiOps[iSub]->res = 0.0;
 Scalar *v1 = dv1.subData(sd[iSub]->localSubNum());
...
GenFetiSolver<Scalar>::getNeighbFGs(int iSub)
{
  ...
  opControl->cset[iSub].locFGs = new Scalar[numRBM*sd[iSub]->interfLen()];
...
GenFetiSolver<Scalar>::getSRMult(int iSub, GenDistrVector<Scalar> *r, GenDistrVector<Scalar> *lambda, Scalar *sv)
{
 ...
 Scalar *alpha = sv + eqNums->firstdof(sd[iSub]->subNum()+gOffset);
...
GenFetiSolver<Scalar>::getSCtMult(int iSub, GenDistrVector<Scalar> *r, Scalar *sv)
{
 int cOffset = subToSub->csize();
 Scalar *lvec = r->subData(iSub);
 Scalar *beta = sv + eqNums->firstdof(sd[iSub]->subNum()+cOffset);
...
GenFetiSolver<Scalar>::getGtMult(int iSub, GenDistrVector<Scalar> *r, Scalar *sv)
{
 ...
 Scalar *lvec  = r->subData(iSub);
 Scalar *locGs = opControl->cset[iSub].locGs;
 Scalar *alpha = sv + eqNums->firstdof(sd[iSub]->subNum());
...
GenFetiSolver<Scalar>::getFCMult(int iSub, GenDistrVector<Scalar> *r, Scalar *sv)
{
 ...
 Scalar *alpha = sv + eqNums->firstdof(sd[iSub]->subNum()+cOffset);
