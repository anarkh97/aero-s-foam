...
C     six-nodes triangle for 1 dof per node described
...
C     The outputs are:
C
C       MM        (6 x 6) computed element matrix. 
C       STATUS    Status character variable.  Blank if no error detected.
C
C=END USAGE
C=BLOCK FORTRAN
      subroutine  TRI6AXIMAS(r, z, p, mm, m)
C
C                   A R G U M E N T S
C
      integer           p, m
      double precision  r(6), z(6)
