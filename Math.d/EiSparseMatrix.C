...
GenEiSparseMatrix<Scalar>::GenEiSparseMatrix(Connectivity *cn, DofSetArray *dsa, int *rCN)
...
template<class Scalar>
GenEiSparseMatrix<Scalar>::GenEiSparseMatrix(Connectivity *cn, DofSetArray *dsa, DofSetArray *c_dsa)
: SparseData(dsa,c_dsa,cn),
  nnz(xunonz[numUncon]),
  unonz(new Scalar[nnz]),
  M(numUncon, numUncon, nnz, xunonz, rowu, unonz)
{
...
{
  // NOTE: it is not necessary to use transpose because M is symmetric
  Eigen::Map< Eigen::Matrix<Scalar, Eigen::Dynamic, 1> > rhs(_rhs.data(),numUncon,1), result(_result.data(),numUncon,1);
  result = M.template selfadjointView<Eigen::Upper>()/* .transpose().evaluate() */ *rhs;
}
