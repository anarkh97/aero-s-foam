...
LinearDriverImpl::solve() {
  ...
  CorrectionNetworkImpl::Strategy correctionStrategy = domain->solInfo().NoForcePita ?
    CorrectionNetworkImpl::HOMOGENEOUS :
    CorrectionNetworkImpl::NON_HOMOGENEOUS;
  ...
  HalfSliceNetwork::Ptr network = new HalfSliceNetwork(
      ...
      schedule.ptr(),
      seedInitializer.ptr(),
      (correctionStrategy == CorrectionNetworkImpl::NON_HOMOGENEOUS) ? seedInitializer.ptr() : NULL);
