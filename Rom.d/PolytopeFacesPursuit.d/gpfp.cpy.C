      ...
      double maxsze, double reltol, bool verbose, bool scaling)
{
  ...
  std::set<long int> activeSet1;    // container for indices of active set of  A
  std::set<long int> activeSet2;    // container for indices of active set of -A
  ...
  for (int col = 0; col < columns; col++) activeSet1.insert(col);
  for (int col = 0; col < columns; col++) activeSet2.insert(col);
  ...
  while (true) {
    ...
    double lam      = 0.;
    bool   set1     = true;
    std::set<long int>::iterator position;   

    //compute the scaled gradient of ||[A,-A]*s - b||_2
    for(std::set<long int>::iterator it = activeSet1.begin(); it != activeSet1.end(); it++){
      double num = S[*it]*A.col(*it).transpose()*r;
      double den = S[*it]*A.col(*it).transpose()*c;
      g1(*it) = num;
      g2(*it) = den;
      double tmp = num/(1-den);
      if(tmp > maxStep && num > 0){
        position = it;
        maxStep  = tmp;
        lam      = 1/tmp;
      }
    ...
    for(std::set<long int>::iterator it = activeSet2.begin(); it != activeSet2.end(); it++){
      double tmp;
      double num;
      double den;
      if (g1(*it) != 0.) {// if we computed it already, dont re-compute
        num = (-1.)*g1(*it);
        den = (-1.)*g2(*it);
        tmp = num/(1-den);
      } else {           // need to compute
        num = S[*it]*(-1.)*A.col(*it).transpose()*r;
        den = S[*it]*(-1.)*A.col(*it).transpose()*c;
        tmp = num/(1-den);
      }
...
*/
      ...
      // update correct set
      if(setKey[krestart] == 1) {
        // replace activeSet1
        activeSet1.insert(dualSet[krestart]);
      } else {  
        // replace activeSet2
        activeSet2.insert(dualSet[krestart]);
      }
      ...
      for(int i = 0; i < krestart; i++){
        c += lambda(i)*r;       
        r -= alpha(i)*B.leftCols(i+1)*D.col(i).head(i+1);
        c -= lambda(i)*r;
      }
      ...
      for(int j = krestart; j < k+1; j++){
        ...
        alpha(curRowCol)   = r.dot(update)*DtGDinv(curRowCol);
        lambda(curRowCol)  = (1-B.col(curRowCol).dot(c))/(B.col(curRowCol).dot(r));
