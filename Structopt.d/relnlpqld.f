...
C
    ...
    2 CONTINUE
C  
      CALL RELNLPQL1(X,XL,XU,F,G,DF,DG,                        
...
C***********************************************************************
      SUBROUTINE RELNLPQL1(X,XL,XU,F,G,DF,DG,  
     1           U,C,D,WA,KWA,ACTIVE,           
     2           M,ME,N,
     2           MMAX,NMAX,MNN2,LWA,LKWA,LACTIV,
     3           ACC,SCBOU,MAXFUN,MAXIT,IPRINT,MODE,IOUT,IFAIL,
...
C
     ...
     4     IFAIL1,MAXFU1,IOUT1
      COMMON/RELNLPQL3/F1,ACC1,SCBOU1,DBDFAC,ZEFAC,RPENO,RPENS,RPENU,     
...
C
    4 CONTINUE
      ...
      LQL1=LQL
      LMER=LMERIT
      CALL RELNLPQL2(MMAX,N,NMAX,X,G,DF,DG,U,XL,XU,C,D,WA(IVMU),
     1     WA(IDEL),     
...
C
      RETURN
      END
      SUBROUTINE RELNLPQL2(MMAX,N,NMAX,X,G,DF,DG,U,XL,XU,DCL,CD,VMU,
...
C
     ...
     1     MN1,MNN1,I1,I,J,MMAX2,MMAX41
      COMMON/RELCMACHE/EPS0
      COMMON/RELNLPQL3/F,ACC,SCBOU,DBDFAC,ZEFAC,RPENO,RPENS,RPENU,
...
C
  ...
  206 CONTINUE
c
       open (46,file="rndvariables.data")
         DO IX=1,N
...
C
      ...
      WRITE(IOUT,*)      
C
C      CALL NLFUNC(M,ME,MMAX,N,F,G,X,ACTIVE(M+1))
C      CALL NLGRAD(M,ME,MMAX,N,F,G,DF,DG,X,ACTIVE,WA(41))
C
   77 CONTINUE
      LSCF=.FALSE.
      LSCG=.FALSE.
  ...
  182 CONTINUE
   ...
   25 CONTINUE
    ...
    3 IF(MODE.EQ.1.OR.MODE.EQ.6.OR.MODE.EQ.3.OR.MODE.EQ.8) VMU(J)=U(J)
      CALL RELMERIT1(IMERIT+3,M,ME,MMAX,N,MNN,NMNN,ACC,RPEN,F,DF,G,DG,        
...
C
   ...
   71 W(I)=XU(I) - X(I)
      ...
      IF (LQL) IW(11)=1
      IW(12)=0
      CALL RELQL0001(M,ME,MMAX,N,NMAX,MNN,DCL,DCLF,DG,G,V,W,DEL,U,
  ...
  154 CONTINUE
   ...
   56 CONTINUE
      ...
      IF (LQL) IW(11)=1
      IW(12)=1
      CALL RELQL0001(M,ME,MMAX,N1,NMAX,MNN2,DCL,DCLF,DG,G,V,W,DEL,U,
     1     IOUT,IFAIL1,IPR,WA(MMAX41),LWAQL,IW(11),LIWQL)
      NQL=NQL+1
      MN1=M+N1+1
      MNN1=M+N1+N
...
C
 ...
 1110 FORMAT(8X,48H**WARNING: AUGMENTED LAGRANGIAN SEARCH DIRECTION)
   92 CALL RELMERIT1(4,M,ME,MMAX,N,MNN,NMNN,ACC,RPEN,F,DF,G,DG,VMU,U,
   ...
   93 DCLF(I)=DPHI(I)
      ...
      IF(LQL) IW(11)=1
      IW(12)=0
      CALL RELQL0001(0,0,MMAX,N,NMAX,MNN2,DCL,DCLF,DG,G,V,W,DEL,U,IOUT,
...
C
 ...
 1510 FORMAT(8X,30HNUMBER OF ACTIVE CONSTRAINTS: ,19X,5HNAC =,I4)
   ...
   70 SUM=SUM + DABS(U(M+I)*V(I)) + DABS(U(MN+I)*W(I))
      ...
      IF (LSCF) FF=F/SCF
C
C      WRITE(IOUT,1460) ITER,FF,SRES,NACT,ILINE,ALPHAO,DEL(N1),SUM
      WRITE(IOUT,*)
      WRITE(IOUT,*)
      WRITE(IOUT,'(A,I5)') 'ITERATION ',ITER
      WRITE(IOUT,'(A)') 
...
C
      ...
      WA(3)=RPENU
      WA(4)=DBLE(ITER)
      CALL RELMERIT1(IMERIT+2,M,ME,MMAX,N,MNN,NMNN,ACC,RPEN,F,DF,G,DG,
     1     VMU,U,X,XL,XU,PHI,DPHI,ACTIVE,LACTIV,WA,4)
      GOTO 99
   87 SUM=ZE
      DO 88 I=1,N
   ...
   97 RPEN(J)=DMIN1(ZEFACU,RPEN(J)*RPENO)
      CALL RELMERIT1(IMERIT+4,M,ME,MMAX,N,MNN,NMNN,ACC,RPEN,F,DF,G,DG,
     1     VMU,U,X,XL,XU,PHI,DPHI,ACTIVE,LACTIV,WA,4)
...
C
   95 CALL RELMERIT1(IMERIT+3,M,ME,MMAX,N,MNN,NMNN,ACC,RPEN,F,DF,G,DG,
     1     VMU,U,X,XL,XU,PHI,DPHI,ACTIVE,LACTIV,WA,4)
      IF (.NOT.LALSD) CALL RELMERIT1(IMERIT+4,M,ME,MMAX,N,MNN,NMNN,ACC,
...
C
 ...
 2810 FORMAT(7X,21H*** LINE SEARCH STEP ,I2,10H: ALPHA = ,D11.4, 
     1     14H, MERIT FCT = ,D13.6)
      CALL RELLINSEA(ALPHAO,ALPHAM,PHI,PROD,AMUE,BETA,ILINE,MAXFUN,
     1     IFLISE,IPR,WA(6),35,IW,10,ACTIVE(ILWLS),5)
 ...
 1170 FORMAT(8X,28HLINE SEARCH SUCCESSFUL AFTER,I3,7H STEPS:,
  ...
  201 CONTINUE
      CALL RELMERIT1(IMERIT+1,M,ME,MMAX,N,MNN,NMNN,ACC,RPEN,F,DF,G,DG,
     1     VMU,U,X,XL,XU,PHI,DPHI,ACTIVE,LACTIV,WA,4)
      GOTO 18
  197 CONTINUE
C
...
C
   ...
   86 DCL(J,I)=DCL(I,J)
      GOTO100
   85 CONTINUE
      CALL RELLDL001(N,NMAX,DCL,CD,ETA,BDEL)
C
...
C
 9999 RETURN
      END
      SUBROUTINE RELLDL001(N,NMAX,CL,D,U,V)
C
C********************************************************************
C
C
...
C
      ...
      END
C
      SUBROUTINE RELLINSEA(ALPHA,ALPHAM,PHI,DPHI,AMUE,BETA,ILINE,MAXFUN,
...
C
 9999 RETURN
      END
      SUBROUTINE RELMERIT1(MODE,M,ME,MMAX,N,MNN,NMNN,ACC,R,F,DF,G,DG,V,
     1     U,X,XL,XU,PHI,DPHI,ACTIVE,LACTIV,WA,LWA)
C
C*********************************************************************
C
C
...
C
      END
      SUBROUTINE RELQL0001(M,ME,MMAX,N,NMAX,MNN,C,D,A,B,XL,XU,
...
C     CALL OF QL0002
C
      CALL RELQL0002(N,M,ME,MMAX,MN,MNN,NMAX,LQL,A,WAR(INW1),
...
C
 ...
 1400 FORMAT(/8X,50H***QL: ACCURACY INSUFFICIENT TO ATTAIN CONVERGENCE) 
      END
C
      SUBROUTINE RELQL0002(N,M,MEQ,MMAX,MN,MNN,NMAX,LQL,A,B,GRAD,G,
