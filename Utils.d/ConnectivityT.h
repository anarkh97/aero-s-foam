...
class ConnectivityT : public BaseConnectivityT<ConnectivityT<_IndexType,_DataType>,DirectAccessT<ConnectivityT<_IndexType,_DataType> > >
{
 ...
 protected:
        bool removeable;     // whether to delete pointer memory or not
	IndexType size;      // size of pointer
	IndexType numtarget; // size of target, number of Connections
	IndexType *pointer;  // pointer to target
	DataType *target;    // value of the connectivity
	float *weight;       // weights of pointer (or 0)
       
 public:
	IndexType getNumTarget() { return numtarget; }
	DataType * getTarget()   { return target; }
	IndexType * getPointer() { return pointer; }

        ConnectivityT() { size = 0; numtarget = 0; pointer = 0; target = 0; weight = 0; removeable = true; }
	...
	size_t read(FILE* f);
  	...
  	void countlink(IndexType from, IndexType to); //DEC
        ...
        virtual ~ConnectivityT();
        void setRemoveable(bool _removeable) { removeable = _removeable; }
        ...
        IndexType *ptr() { return pointer; }
        DataType *tgt() { return target; }
...
template<typename IndexType, typename DataType>
DataType *
ConnectivityT<IndexType,DataType>::operator[](IndexType i) { return target + pointer[i]; }
 ...
 * as a ConnectivityT without constructing the real ConnectivityT, but through clever use
...
#include <Utils.d/ConnectivityT.C>
#endif
