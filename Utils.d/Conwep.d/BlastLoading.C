...
double BlastLoading::Conwep::Blast(const BlastLoading::BlastData& P,
  ...
  double f[3] = {P.x0[0] - x[0], P.x0[1] - x[1], P.x0[2] - x[2] };
  double R = sqrt( f[0]*f[0]+f[1]*f[1]+f[2]*f[2] );
  f[0] /= R;
  f[1] /= R;
  f[2] /= R;
  R *= 3.2808399; // meters to feet
  double posCosine = n[0]*f[0]+n[1]*f[1]+n[2]*f[2];
  
  double arrivalTime,positivePhaseDuration,incidentImpulse,
    reflectedImpulse,incidentPressure, reflectedPressure;
  double a,b;

  Conwep::Params( P,R, arrivalTime,positivePhaseDuration,incidentImpulse, 
		  reflectedImpulse,incidentPressure, reflectedPressure,
		  a,b );

  double ts = (t - P.t0)*1000.0; // time in ms
  double p = Conwep::Pressure(ts,arrivalTime,
...
void BlastLoading::Conwep::Params(const BlastLoading::BlastData& P,
  ...
  if ((++cnt) < 122) {
    det << "R = " << R << " Arrival time = " << arrivalTime << "\n"
	      << "Pos. Phase Dur = " << positivePhaseDuration << "\n"
	      << "IncidentImpulse = " << incidentImpulse << "\n"
	      << "ReflectedImpulse = " << reflectedImpulse << "\n"
	      << "IncidentPressure = " << incidentPressure << "\n"
	      << "ReflectedPressure = " << reflectedPressure << std::endl;
  }
...
BlastLoading::BlastData BlastLoading::myData = { {-2.0, 0.0, -0.6}, -0.1e-3,
